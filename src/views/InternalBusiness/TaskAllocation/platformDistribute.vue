        ...
        <el-form>
            <el-form style="width: 100%; display: flex">
                ...
                <div class="block" style="margin-left: 8px">
                    <el-date-picker v-model="timenode" placeholder="选择日期" type="date" align="right">
                    </el-date-picker>
                </div>
                <el-form-item>
                    <el-button icon="el-icon-search" size="mini" @click="handleQuery" style="
              margin-left: 8px;
            ...
            <el-form-item>
                <div class="letf-items" style="float: left">
                    ...
                    <el-button type="danger" icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDeleteAll">删除</el-button>
                    <el-button type="warning" size="mini" @click="findNumber">查看平台数量</el-button>
                </div>
            </el-form-item>
        </el-form>
        ...
        <el-table v-if="merchantList" v-loading="loading" :data="merchantList" @selection-change="handleSelection" max-height="600">
            ...
            <el-table-column label="有效期" align="center" prop="end_time">
                <template slot-scope="scope">
                    ...
                    {{
            [
              new Date(scope.row.end_time * 1000).getFullYear(),
              new Date(scope.row.end_time * 1000).getMonth() + 1,
              new Date(scope.row.end_time * 1000).getDate()
            ].join("-")
          }}
                </template>
            </el-table-column>
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    ...
                    <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.$index, merchantList)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!--分页组件-->
        <el-pagination v-show="total > 0" :total="total" @size-change="sizeChange" @current-change="currentChange" :current-page="queryParams.pageNum" :page-size="queryParams.pageSize" style="margin-top: 8px" layout="total, prev, pager, next, sizes" />
        <!-- 新增 -->
                    ...
                    <el-col :span="14">
                        <el-form-item label="员工">
                            <el-select v-model="staff" placeholder="请选择" clearable @change="changeS(staff)">
                                <el-option v-for="item in staff_list" :key="item.user_id" :label="item.user_nick" :value="item.user_id" :disabled="item.disabled">
                                </el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                    <el-col :span="14">
                        <el-form-item label="平台">
                            <el-select v-model="plat" multiple placeholder="请选择">
                                <el-option v-for="item in plat_items" :key="item.value" :label="item.label" :value="item.value" :disabled="item.disabled">
                                </el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
...
export default {
        ...
        project: function () {
            for (let index in this.projects) {
                if (this.projects[index].value == this.project) {
                    ...
                    getShowStaff(params).then((res) => {
                        console.log(res);
                        this.staff_list = res.items;
                        this.plat_items = res.plat_items;
                    });
                }
            }
        },
    ...
    methods: {
        ...
        getList() {
            ...
            getStaffPlat(params).then((res) => {
                // console.log(res);
                this.merchantList = res.items;
                this.loading = false;
                this.total = res.data_num; //总条数
        ...
        /** 新增中的提交按钮 */
        submitForm: function () {
            console.log(this.plat,this.user_id,this.project,this.times)
            if (
                this.plat == "" ||
                this.user_id == "" ||
                this.project == "" ||
